...
def predict(Model, adduct_SET, dataset, adduct):
    ...
    * y_pred : CCS prediction results of the model
    ...
    y_pred = []
    p_bar = tqdm(
        range(len(bar)), desc="Predictions", total=len(bar), ncols=90, file=sys.stdout
    )
...
def Mymodel(DataSet, adduct_SET):
    ...
    * model : The constructed SiGMA model
    ...
    output = ECCConv(16, Kernel_Network, activation="relu", kernel_regularizer="l2")(
        [X_in, A_in, E_in]
    )
    output = ECCConv(16, Kernel_Network, activation="relu", kernel_regularizer="l2")(
        [output, A_in, E_in]
    ...
    DenseS = Dense9(
        Dense8(Dense7(Dense6(Dense5(Dense4(Dense3(Dense2(Dense1(concat))))))))
    )
    ...
    * Model : The constructed SiGMA model
    ...
    for epoch in range(EPOCHS):
        ...
        loader_tr = BatchLoader(
            dataset_tr_2, batch_size=BATCHS, epochs=1, shuffle=False
        )
        loader_tr_data = ()
        ltd_index = 0
        # The Graph data of each molecule is spliced with adduct data
        for i in loader_tr.load():
            adduct_one_hot = [
                one_of_k_encoding_unk(adduct_tr_2[ltd_index + ltd_index_i], adduct_SET)
                for ltd_index_i in range(len(i[1]))
            ]
            adduct_one_hot = np.array(adduct_one_hot)
            one_sample = ((adduct_one_hot, i[0][0], i[0][1], i[0][2]), i[1])
