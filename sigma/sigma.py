...
def Model_prediction(ifile, ParameterPath, mfileh5, ofile, Isevaluate=0):
    ...
    * Isevaluate    : Evaluate ?
    ...
    smiles, adduct, ccs, Coordinate = Generating_coordinates(
        smiles, adduct, ccs, param.All_Atoms
    )
    ...
    data = {"SMILES": smiles, "Adduct": adduct, "True CCS": ccs, "Predicted CCS": re}
    df = DataFrame(data)
...
def Model_train(
    ifile, ParameterPath, ofile, EPOCHS, BATCHS, Vis, All_Atoms=[], adduct_SET=[]
):
    """
    * Train
    *
    ...
    * ofile         : File path where the model is stored
    ...
    # Training Model
    ECC_Model = train(
        ECC_Model, DataSet, adduct, adduct_SET, EPOCHS=EPOCHS, BATCHS=BATCHS, Vis=Vis
    )
    # Save model
